...
namespace
{
		...
		const FCardEffectSpec& CardEffectSpec)
	{
		...
		JargonEffectSpec.SummonedUnitDefinition = CardEffectSpec.SummonedUnitDefinition;
		...
		case ECardEffectOperation::ApplyFreeze:
			JargonEffectSpec.Operation = EJargonEffectOperation::ApplyFreeze;
			JargonEffectSpec.Delivery = JargonEffectSpec.Radius > 0
				? EJargonEffectDelivery::UnitsInRadius
				: EJargonEffectDelivery::ExplicitUnit;
			JargonEffectSpec.TargetFilter = EJargonEffectTargetFilter::EnemyToSource;
		...
		case ECardEffectOperation::GainElementCharge:
			JargonEffectSpec.Operation = EJargonEffectOperation::GainElementCharge;
			JargonEffectSpec.Delivery = EJargonEffectDelivery::Self;
			JargonEffectSpec.TargetFilter = EJargonEffectTargetFilter::SourceOnly;
			JargonEffectSpec.ElementType = CardEffectSpec.ElementType;
			break;
	...
	TArray<FJargonEffectSpec> ConvertCardEffectArrayToJargonEffectSpecs(
		const UCardDefinition* Card,
		const TArray<FCardEffectSpec>& CardEffectSpecs)
	{
		...
		JargonEffectSpecs.Reserve(CardEffectSpecs.Num());
		for (const FCardEffectSpec& CardEffectSpec : CardEffectSpecs)
		{
	...
	{
		return Card ? ConvertCardEffectArrayToJargonEffectSpecs(Card, Card->Effects) : TArray<FJargonEffectSpec>();
	}
	...
	void EmitElementalBonusTriggeredCue(
		...
		const FCardElementalBonusGroup& BonusGroup,
		int32 BonusIndex,
		...
		bool bSpentCharges)
	{
		...
		Cue.ElementType = BonusGroup.ElementType;
		...
		Cue.TextOverride = FText::Format(
			...
			GetElementDisplayText(BonusGroup.ElementType));
	...
	FJargonEffectTrace* OutTrace)
{
	...
	for (int32 BonusIndex = 0; BonusIndex < Card->ElementalBonusGroups.Num(); ++BonusIndex)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Card '%s' elemental bonus group %d skipped because it has no BonusEffects."),
				*Card->DisplayName.ToString(),
				BonusIndex);
			continue;
		}
		...
		const TArray<FJargonEffectSpec> BonusJargonEffects = ConvertCardEffectArrayToJargonEffectSpecs(Card, BonusGroup.BonusEffects);
		...
		{
			if (BonusIndex < Card->ElementalBonusGroups.Num() - 1)
			{
	...
	FCardResolveResult& OutResult)
{
	...
	case ECardEffectOperation::ApplyFreeze:
		return ResolveApplyFreezeEffect(Card, EffectSpec, Context, OutResult);
	...
	FCardResolveResult& OutResult)
{
	...
	if (EffectRadius > 0)
	{
		...
		for (const TWeakObjectPtr<ABattleUnit>& TargetPtr : TargetsToDamage)
		{
			...
			TargetUnit->ApplyDamageFromSource(DamageAmount, Context.SourceUnit);
...
bool FCardResolver::ResolveApplyFreezeEffect(
	const UCardDefinition* Card,
	const FCardEffectSpec& EffectSpec,
	const FCardResolveContext& Context,
	FCardResolveResult& OutResult)
{
	...
	const int32 FreezeTurns = Card->GetConfiguredValueForEffect(EffectSpec);
	if (FreezeTurns <= 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("Card '%s' has an ApplyFreeze effect with no positive freeze duration."),
			*Card->DisplayName.ToString());
		return false;
	}
	...
	if (EffectRadius > 0)
	{
		...
		for (ABattleUnit* CandidateUnit : AllUnits)
		{
			...
			// Freeze is treated as a hostile stasis effect for now.
			if (CandidateUnit->GetTeam() == Context.SourceUnit->GetTeam())
			{
				continue;
			}
			...
			if (CandidateUnit->GetCurrentTile() &&
				GridBoard->AreTilesWithinRange(CandidateUnit->GetCurrentTile(), CenterTile, EffectRadius))
			{
				CandidateUnit->ApplyFreeze(FreezeTurns);
				bAppliedAnyFreeze = true;
			}
		}

		return bAppliedAnyFreeze;
	}
	...
	TargetUnit->ApplyFreeze(FreezeTurns);
	return true;
}
	...
	FCardResolveResult& OutResult)
{
	...
	{
		TargetUnit->ApplyDamageFromSource(CollisionDamage, Context.SourceUnit);
		bResolvedAnyPushEffect = true;
	}
	...
	FCardResolveResult& OutResult)
{
	...
	auto ApplyChainPayload = [&](ABattleUnit* TargetUnit)
	{
		...
		{
		case ECardEffectOperation::ChainDamage:
			TargetUnit->ApplyDamageFromSource(EffectValue, Context.SourceUnit);
			break;
